...
description: A 股行情取数对照表，短线专用 + 盘中实时 + 盘后复盘全覆盖。任何需要"当前盘面""今天""现在""实时""复盘""龙虎榜""昨天涨停今天怎么走""为什么涨停""主线是什么""连板天梯""RPS三线红""三线红池数"的场景先查这张表，直接调可用接口，不要逐个试错。涵盖涨停归因、题材聚集度、连板天梯、情绪周期、封板细节、炸板率、弱转强验证、历史分钟K线、日内形态量化、分钟资金流、龙虎榜席位、RPS三线红（读本机通达信，不联网）的具体接口映射和已知故障。
---
...
## 一律优先用 `live-market`（自建，**73 个工具**，实测最稳）

脚本 `live_market.py`（实时基础）+ `live_market_ex.py`（股池/历史/盘后）
+ `live_market_ths.py`（短线专用）+ `live_market_kaipanla.py`（开盘啦）
+ `rps_local.py`（RPS 三线红本机直算）+ `rps_pool.py`（名单层），MCP 名 `live-market`。
数据源东财 push2delay / push2ex / push2ex异动 / datacenter-web + 同花顺 data.10jqka
+ 腾讯 + 新浪 + 开盘啦 apphis.longhuvip.com，多源自动降级，全部免费无需注册。
第 7 指标 RPS 那三个工具**不联网**，直读本机通达信的 `extdata` + `lday`。
...
开盘啦那 28 个另有独立回归：`_verify_kpl_all.py` 34 项探针，2026-08-05 盘后重跑
**真数据 20 / 明确缺数 14 / 异常 0，漏到 stdout 0 字节**。

⚠️ **开盘啦不当主源。** 它当日有多个端点静默返回 0（不报错），会把火热盘面
读成冰点。逐维度的选源结论见 `scripts\开盘啦集成说明.md` 的「按维度选源」，
简版：情绪历史序列/大单分时/百日新高用开盘啦（其他源没有），
涨停池/连板高度/当日情绪/龙虎榜一律用东财或同花顺。
...
| **南京路 7 指标**（情绪定位） | `get_sentiment_7indicators` | 7 项各带今日值 + 近5/10日均值 + 近10/60日分位数。前 6 项联网取，第 7 项读本机（见下） |
...
### 7 指标第 7 项：RPS 三线红（不联网，读本机通达信）

前 6 项（涨停数/跌停数/连板数/封板率/涨跌家数比/市场高度）走 `get_sentiment_7indicators`
自己联网取。**第 7 项是中期强势股存量，联网取不到 —— 但本机能直接算**。

| 你要什么 | 调什么 | 返回要点 |
|---|---|---|
| **算今天的三线红** | `rps_local_calc(save=True)` | 直读 `C:\new_tdx` 的 `extdata`+`lday`，返回名单 + 过三线RPS数 / 被HHV砍掉数 |
| 补历史（第一次用先跑） | `rps_local_backfill(days=60)` | 本机有 4235 个可算日，60 天约 32 秒 |
| 本机能算哪些日期 | `rps_local_dates` | 日期范围 + **日线对齐检查**（扩展数据更新了日线没跟上会让池子假性缩水） |
| 趋势强度榜 | `rps_pool_stat` | 每只票的加入日/入连续天数/入总天数/入池次数/距60天第一次 |
| 日间进出 | `rps_pool_diff` | 新进榜/掉出榜/连续在榜/行业增减 |
| 通达信导出的名单入库 | `rps_pool_scan` | 走导出那条路时才用，通常不需要 |

三套阈值口径：`fine`（默认，120>93 250>95 50>90）/ `base`（三线均>90）/ `oneil`（>87），
外加 `H/HHV(HIGH,150)>0.85`。**换口径就是换一整条曲线，前后不可比**，
要换得 `backfill --overwrite` 整批重算。

⚠️ **池子只有个位数不是故障。** 2026-08-05 精细版：过三线 RPS 65 只，
57 只被 `H/HHV(150)>0.85` 砍掉，剩 8 只。150 个交易日往前正好盖住 2-3 月那波高潮。
「过 RPS 数」与「最终数」的差距本身就是读数。轨迹：`05-13` 149 只 → `08-05` 8 只。

⚠️ **绝对只数没有通用含义**（阈值是自己定的），看的是近10/60日分位数、环比扩容缩容、
以及逐只票的在榜天数。**板块三线红（BKRPS 编号 7–11）本机没装，不算也不假装能算**，
作者第 31 页自述「我没有特别挖掘这方面」。我给的「行业分布 TOP5」是从个股反推的
东财行业标签，**和板块 RPS 不是一回事，不能互相印证**。
...
```bash
python _知识库系统/scripts/rps_local.py save
```
...
| **中期强势股存量 / 三线红** | `rps_local_calc` 的池数 + `rps_pool_stat` 的在榜天数（不联网，读本机通达信） |
| **趋势强度**（作者第 26 页「按出现次数做统计」） | `rps_pool_stat` 的入总天数 / 入池次数，比池数更稳（不受阈值平移影响） |
