...
def main() -> int:
    ...
    quality += [
        ...
        "- **`防爆头2.0` 不建块**：被 `20260418` 版覆盖约 90%（各段长门槛下 89.2%~90.4%）。"
        "**对照集是「全部已建块文档」而非单一目标** —— 只对 `20260418` 算独有 2434 字（16.4%），"
        "对 `20260418`+`新日期拆解` 两篇一起算降到 1576 字，其中 120 字是重复 5 次的平台头，"
        "**实质仅 3 段 1426 字**且都是 `20260418` 同一章节的改写版（「暴利」→「正反馈」）。"
        ...
        "⚠️ **`date_precision` 只在 jsonl 里，没有进索引表** —— `build_index.py` 的 chunks "
        "表不含此列，走 `query_kb.py` 检索时取不到，要判断日期可信度只能回读 jsonl。",
        ...
        "- **推广联系方式已剔除**：`一手拼课vx：hzp9512055818`、`一手拼课QQ865154818` 等 16 处，"
        "行内剔除不删整段（部分嵌在正文句子中间）。"
        "⚠️ 但要说清：这 16 处**全部落在不建块的那几篇里**，入库的 172 块本来就没有推广内容，"
        "所以这项清洗对入库正文的实际贡献是 0。",
    ...
    # **22 行注释全部抹掉**（包括 smoke_query 字段的用法说明、panfeng 那段「为什么
    ...
    print(
        "提示：本导入器不再自动回写 config/sources.yaml（那会抹掉 22 行注释）。\n"
