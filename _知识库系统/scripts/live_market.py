...
def _price_cap(code: str, name: str = "") -> int:
    """
    该股的涨跌幅上限（百分比）。北交所 30，创业板/科创板 20，其余主板 10。
    新股上市首日无限制，单独识别（名称带 N 或 C 前缀）。

    **ST 不是一刀切 5%，跟着自己所在板块走。** 2026-08-04 用东财官方涨停价
    （单票接口 f51）对涨幅榜里 9 只 ST 逐只反推 (涨停价/昨收-1)，9/9 一致：

      688646 ST逸飞   科创板 昨收 60.00 涨停价 72.00 → 20.0%
      300167 ST迪威迅 创业板 昨收  3.85 涨停价  4.62 → 20.0%
      301139 *ST元道  创业板 昨收  3.79 涨停价  4.55 → 20.05%
      002501 *ST利源  主板   昨收  1.25 涨停价  1.38 → 10.4%
      000609 *ST中迪  主板   昨收  9.06 涨停价  9.97 → 10.04%
      603838 *ST四通  主板   昨收  7.70 涨停价  8.47 → 10.0%
      600340 *ST华幸  主板   昨收  1.06 涨停价  1.17 → 10.38%
      600818 ST中路   主板   昨收  6.63 涨停价  7.29 → 9.95%
      600381 *ST春天  主板   昨收  3.78 涨停价  4.16 → 10.05%

    偏离 10/20 整数的是四舍五入：低价股一分钱的最小变动占比大，
    1.25 × 1.10 = 1.375 进位到 1.38，反推就成了 10.4%。所以判定必须留容差。

    上一版写死 `if "ST" in name: return 5`，后果是 ST逸飞涨 13.33%、
    ST迪威迅涨 7.79% 都被当成涨停计入。这个 bug 是
    `live_market_health.cross_check()` 第一次运行就抓出来的。

    ⚠️ 主板 ST 按市场常规规则应是 5%，但东财官方涨停价字段给的是 10%
    （且 002501 当日确实涨了 10.40% 并收在该价，5% 上限下不可能发生）。
    这里以实测数据为准；ST 判定另有单票核实兜底，见 limit_up_pool()。
    """
...
def _verify_limit_up(code: str, price) -> bool | None:
    """
    用东财**单票接口的官方涨停价**核实是否真封板。
    返回 True/False；涨停价取不到时返回 None（无法判定，按规则值走）。

    单票接口的 f51 是真涨停价；clist 列表接口的 f51 **不是**
    （实测返回 6829262880.84 这种量级的值，是别的含义），所以只能逐只查。
    """
    q = quote_em(code)
    zt = (q or {}).get("涨停价")
    if not isinstance(zt, (int, float)) or not isinstance(price, (int, float)):
        return None
    return abs(price - zt) < 0.005      # 价格保留两位小数，半分钱内算相等


ST_VERIFY_MAX = 15    # ST 逐只核实的上限，防止极端行情下拖慢整个调用


def limit_up_pool(limit: int = 120) -> list[dict]:
    """
    实时涨停池。按各板块自己的涨停幅度判定（主板 10 / 创业科创 20 / 北交 30），
    不是一刀切 9.8%。新股（N/C 开头）排除，它们没有涨停概念。

    **ST 股不走规则判定，逐只用东财官方涨停价核实**（最多 15 只，约 +1.8 秒）。
    原因：ST 的涨跌幅上限用代码前缀推不出来。2026-08-04 实测 9 只 ST，
    主板的隐含上限是 9.95-10.38%、创业科创的是 20.0-20.05%，
    而上一版代码写死 5%，导致 ST逸飞（涨 13.33%，科创板 20cm 未封板）
    和 ST迪威迅（涨 7.79%）被误计为涨停。

    ⚠️ 已知盲区：如果某只 ST 的真实上限是 5%，它的涨幅只有 5% 左右，
    会落在涨幅榜扫描窗口（约前 700 名，最低约 7%）之外，本函数扫不到它。
    2026-08-04 实测的 9 只 ST 里没有 5% 上限的，所以没为这个情况加深翻页；
    真要覆盖得改成全市场扫描，代价是每次多翻十几页。

    与东财官方涨停池（`live_market_ex.zt_pool`）的口径差异，实测 2026-08-04：
      - 官方池**系统性不含 ST**（池内 138 只，ST 数 0），本函数含真封板的 ST
      - 收盘封在涨停价但盘中反复炸板的票（如高争民爆，炸板 26 次）
        官方归入炸板池而不在涨停池，本函数按「现价==涨停价」计入
    两边都不算错，问的不是同一个问题。要和官方对齐就用 zt_pool()。

    比 akshare 的 stock_zt_pool 强的地方：这个是盘中实时的，
    akshare 那个涨停池盘中数据经常滞后或不可用。
    """
    out, newly, st_pending = [], [], []
    ...
    for page in range(1, 8):
        ...
        for x in rows:
            ...
            if cap == 999:
                ...
                # ST 的上限推不出来，先攒起来，翻页结束后逐只用官方涨停价核实
                row["板制"] = "ST待核实"
                st_pending.append(row)
            elif pct >= cap - 0.3:  # 留 0.3 容差，四舍五入导致的 9.98 也算涨停
                out.append(row)
        if page_max < 9.7 or len(out) >= limit:
            break

    # ST 逐只核实。核实不了的**不计入涨停**，宁可少报也不虚报家数。
    for row in st_pending[:ST_VERIFY_MAX]:
        ok = _verify_limit_up(row["代码"], row["最新价"])
        if ok:
            row["板制"] = "ST(已核实封板)"
            out.append(row)
