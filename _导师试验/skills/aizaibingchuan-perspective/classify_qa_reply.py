...
# ⚠️ 下面三个模式是本项目「借楼日志」判定的唯一定义。
# extract_intraday_log.py 从本模块 import 它们，不要各自复制一份 ——
# 2026-08-09 就因为两边正则不同步（一边有「竞价」一边没有）导致计数差 261 条。

# 问句是纯占位：只有「借楼」及少量修饰
PLACEHOLDER_Q = re.compile(r"^[\s\w川哥神大爱看兄老师，,。\.]*借楼[\s，,。\.！!~]*$")
# 答句开头是借楼或时间戳（14.04借楼 / 午盘借楼 / 竞价借楼 / 10.06借楼）
LOG_A = re.compile(r"^\s*(?:\d{1,2}[.:：]\d{1,2}\s*)?(?:早盘|午盘|尾盘|盘中|竞价)?借楼")
# 剥掉开头的借楼标记，取出正文
LOG_PREFIX = re.compile(
    r"^\s*(?:\d{1,2}[.:：]\d{1,2}\s*)?(?:早盘|午盘|尾盘|盘中|竞价)?借楼[一用补充再加一条：，,\s]*"
)
