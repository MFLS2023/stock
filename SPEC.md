...
###### 该门槛已到期并执行（2026-08-04，9175 块）

爱在冰川接入把语料从 3362 推到 9175 块（2.7 倍），本节记录实测结果与所做的一轮优化。

**先说结论**：耗时确实按语料等比上涨了，量级仍可用，但已从"几十毫秒"进到"上百毫秒"。
门槛列的三个备选（短词专用索引、换分词器、候选池分层）**一个都没引入**——先做了一个
更便宜的改动就拿到 1.4–1.7 倍，那三项留给下一轮。

**改动：短词召回从「查 `chunks_fts`、三列各一条 UNION」改为「查 `chunks` 表、三列一次
OR 扫完」。** 两处开销同时省掉：① FTS5 内容表按段压缩存放，扫它要一路解压，而短词模式
凑不出可用 gram，两边都是全表扫，走 FTS 白付解压成本；② 三条 UNION 子查询就是扫三遍，
而三列在同一行上，一次扫描全都读到了。

| 查询 | 改前 | 改后 | 提速 |
|-----|-----|-----|-----|
| `龙头` | 203.2ms | 119.7ms | 1.70x |
| `竞价` | 169.4ms | 100.0ms | 1.69x |
| `低吸` | 184.8ms | 112.1ms | 1.65x |
| `龙头 竞价 弱转强` | 389.7ms | 246.0ms | 1.58x |
| `板块` | 215.5ms | 150.0ms | 1.44x |
| `AI` | 246.8ms | 176.2ms | 1.40x |
| `弱转强`（长词对照） | 9.1ms | 9.4ms | 0.97x |

末行是对照组：长词走 MATCH 索引，本改动不碰那条路径，落在噪声内。

**等价性是前提，逐词验收过**：139 个词（两字常用词、同义词表全量、ASCII 大小写四写法、
希腊 `ΑΑΑ`/`ααα`/`ΣΣΣ`、全角 `ＡＡＡ`、GLOB 元字符 `*`/`?`/`[` 及其组合、只在 title 或
author 出现的词、单字、标点、空串）逐词比对两种写法的 chunk_id 集合，**不同的 0 个**。
地基同样验了：两表均 9175 行、chunk_id 集合相同、`text`/`title`/`author` 三列逐行不一致
0 行。端到端另比对 19 个查询（全量 + `limit 8` + 五个 `--source` + 三个 `--author`），
结果逐条相同。

**没有改排序决胜键，且试过之后明确否决。** 排序第 4 层是 `chunk_id` 字典序升序，而
`azbc-` 在字母序上排在 `fulibei-`/`nanjinglu-`/`panfeng-`/`tulip-` 全部之前，所以并列组
内爱在冰川系统性占先——`FIELD_CAPS` 把正文命中截在 8 次，命中 ≥8 次的块评分完全相同，
`赚钱效应` 有 1759 块落在同一个并列组里。试过换成 `blake2b(chunk_id)` 稳定哈希决胜
（确定性、幂等、9175 个 id 零碰撞、各来源哈希首字符均值 7.24–7.66 对理想 7.5，与
`source_id` 无关，不构成配额）。**实测收益只有 2%，且一半的词更差**：12 个词的"前 50
分布与全量占比偏离度"合计 227.5 → 222.4，`板块` 改善 49%、`打板` 48%，但 `赚钱效应`
恶化 70%、`竞价` 恶化 25%，`龙头`/`筹码`/`低吸`/`情绪` 完全不变。

更要紧的是**那个"偏离度"指标本身就违反 §不设来源配额**：它默认"前 50 应按全量占比
分配"，而本文档明确记着各来源强项不同（`筹码` 郁金香最多、`龙头` 复利杯最多）。实测
`筹码` 前 8 是郁金香 8 条、`龙头` 前 8 是复利杯 8 条、`打板` 前 8 是爱在冰川 8 条，
**三者都是 8/8 正文真含**——那是正确结果，不是偏斜。所以决胜键保持 `chunk_id`。

**下一轮的门槛**：`板块` 150ms、三词混合 246ms 已是当前上限附近，语料再翻倍就要上秒。
届时只能动召回结构（两字词倒排表最直接），调参和换写法的空间已经用完。
...
| 优先级 | 事项 | 前置条件 |
|-------|------|---------|
| ~~P0~~ | ~~`build_index.py` 无条件扫 `source_libraries/`，不按 `status` 过滤~~ | **已修（2026-08-04）**：`load_source_registry()` 读 `sources.yaml`，只导入 `integrated` / `integrated_first_pass`，跳过的记入 `metadata.skipped_sources` |
| P1 | `claim_type` 各来源均为固定值，元数据过滤失效 | 需定义标注规则。**2026-08-08 实测确认仍存在**：`claim_type` 9149/9175 是 `opinion_or_case`（另 20 张方法卡 `rule`、6 张分歧卡 `opinion`）、`market_regime` 8963/9175 是「未标注」。拿这两个字段过滤等于无效 |
| P1 | 复利杯闲聊未标记，检索夹杂无关内容 | **2026-08-08 实测确认存在但不宜简单标记**：`直播间` 命中 150 块（10.2%）、`麦` 115、`连麦` 47。但抽样看闲聊与干货**混在同一块**里（`fulibei-001-p001-c03` 既有「欢迎兄弟们来到直播间」也有「你先讲一讲你复盘和你的计划」），整块标为闲聊会连带丢干货。要修得先做块内切分，不是加个标签的事 |
| ~~P2~~ | ~~方法卡仅 20 张且全来自复利杯，无 `status` 字段~~ | **status 已加、20 张已于 2026-08-08 审批进索引**（走 `reports\方法卡审批清单.md` + `apply_method_review.py`）。**剩余问题是数量**：仍只有 20 张且全来自复利杯，另四个来源一张都没有 |
| P1 | 方法卡的 16 条 `user_questions` 全是「怎么判断/怎么执行」，卡本身答不了 | 先查库确认这些 How 是「无人讲过」还是「讲过但搜不到」，再决定加来源还是上语义检索 |
